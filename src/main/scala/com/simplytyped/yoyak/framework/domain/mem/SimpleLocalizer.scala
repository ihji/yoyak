...
import com.simplytyped.yoyak.framework.domain.mem.StdObjectModel.AbsObject
...
class SimpleLocalizer[
...
] extends Localizable[MemDom[A, D]] {
  ...
  private def moveObjects(
      oldMem: MemDom[A, D],
      targets: Set[AbsAddr],
      newMem: MemDom[A, D] = MemDom.empty
  ): MemDom[A, D] = {
    if (targets.isEmpty) newMem
    else {
      val (newMemory, newTargets) =
        targets.foldLeft(newMem, Set.empty[AbsAddr]) { case ((m, s), a) =>
