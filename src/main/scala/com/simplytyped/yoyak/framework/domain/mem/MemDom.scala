...
class MemDom[A <: Galois: ArithmeticOps, D <: Galois: LatticeWithTopOps]
    extends StdObjectModel[A, D, MemDom[A, D]] {
...
object MemDom {
  def empty[A <: Galois: ArithmeticOps, D <: Galois: LatticeWithTopOps] =
    new MemDom[A, D]

  def widening[
      A <: Galois: Widening: ArithmeticOps,
    ...
    new Widening[GaloisIdentity[MemDom[A, D]]] {
      ...
      override def <>(x: MemDom[A, D], y: MemDom[A, D]): MemDom[A, D] = {
        val newRawMap = MapDom
          .widening[AbsAddr, GaloisIdentity[AbsValue[A, D]]]
          .<>(x.rawMap, y.rawMap)
        val newMemDom = new MemDom[A, D]
  ...
  def ops[A <: Galois: ArithmeticOps, D <: Galois: LatticeWithTopOps] =
    new LatticeOps[GaloisIdentity[MemDom[A, D]]] {
      ...
      override def partialCompare(
        ...
        MapDom
          .ops[AbsAddr, GaloisIdentity[AbsValue[A, D]]]
          .partialCompare(lhs.rawMap, rhs.rawMap)
      }

      override def \/(lhs: MemDom[A, D], rhs: MemDom[A, D]): MemDom[A, D] = {
        val newRawMap = MapDom
          .ops[AbsAddr, GaloisIdentity[AbsValue[A, D]]]
          .\/(lhs.rawMap, rhs.rawMap)
        val newMemDom = new MemDom[A, D]
        newMemDom.rawMap = newRawMap
        newMemDom
      }
