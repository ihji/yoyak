...
class MapDom[K, V <: Galois: LatticeOps] {
  ...
  def weakUpdate(kv: (K, V#Abst)): MapDom[K, V] =
    add(kv._1 -> valueOps.\/(get(kv._1), kv._2))
  ...
  def foldLeft[T](init: T)(f: (T, (K, V#Abst)) => T): T =
    rawMap.foldLeft(init)(f)
...
object MapDom {
  ...
  def widening[K, V <: Galois: LatticeOps: Widening] =
    new Widening[GaloisIdentity[MapDom[K, V]]] {
      val widening = implicitly[Widening[V]]
      override def <>(x: MapDom[K, V], y: MapDom[K, V]): MapDom[K, V] = {
        val newDom = new MapDom[K, V]
        newDom.rawMap = y.rawMap.map { case (k, v) =>
          val oldV = x.get(k)
  ...
  def ops[K, V <: Galois: LatticeOps] =
    new LatticeOps[GaloisIdentity[MapDom[K, V]]] {
      val valueOps = implicitly[LatticeOps[V]]
