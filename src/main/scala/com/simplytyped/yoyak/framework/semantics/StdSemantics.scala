...
trait StdSemantics[A<:Galois,D,Mem<:MemDomLike[A,D,Mem]] extends AbstractTransferable[GaloisIdentity[Mem]] {
  ...
  override protected def transferIdentity(stmt: Identity, input: Mem)(implicit context: Context) : Mem = {
    val (rv,output) = eval(stmt.rv,input)
    output.update(stmt.lv,rv)
  }
  override protected def transferAssign(stmt: Assign, input: Mem)(implicit context: Context) : Mem = {
    val (rv,output) = eval(stmt.rv,input)
    output.update(stmt.lv,rv)
  }
  override protected def transferInvoke(stmt: Invoke, input: Mem)(implicit context: Context) : Mem = input
  override protected def transferIf(stmt: If, input: Mem)(implicit context: Context) : Mem = input
  override protected def transferAssume(stmt: Assume, input: Mem)(implicit context: Context) : Mem = input
  override protected def transferReturn(stmt: Return, input: Mem)(implicit context: Context) : Mem = input
  override protected def transferNop(stmt: Nop, input: Mem)(implicit context: Context) : Mem = input
  override protected def transferGoto(stmt: Goto, input: Mem)(implicit context: Context) : Mem = input
  override protected def transferEnterMonitor(stmt: EnterMonitor, input: Mem)(implicit context: Context) : Mem = input
  override protected def transferExitMonitor(stmt: ExitMonitor, input: Mem)(implicit context: Context) : Mem = input
  override protected def transferThrow(stmt: Throw, input: Mem)(implicit context: Context) : Mem = input
  ...
  protected def evalConstant(v: Value.Constant, input: Mem)(implicit context: Context) : (AbsValue[A,D],Mem) =
    (AbsArith[A](arithOps.lift(v)),input)

  protected def evalLoc(l: Value.Loc, input: Mem)(implicit context: Context) : (AbsValue[A,D],Mem) =
    (input.get(l),input)

  protected def evalBinExp(binExp: Value.BinExp, input: Mem)(implicit context: Context) : (AbsValue[A,D],Mem) = {
    binExp match {
      case e : Value.CompBinExp => evalCompBinExp(e,input)
      case e : Value.CondBinExp => evalCondBinExp(e,input)
    }
  }
  protected def evalCompBinExp(compExp: Value.CompBinExp, input: Mem)(implicit context: Context) : (AbsValue[A,D],Mem) = {
  ...
  protected def evalCondBinExp(condExp: Value.CondBinExp, input: Mem)(implicit context: Context) : (AbsValue[A,D],Mem) = {
    (AbsTop,input)
  }
}
