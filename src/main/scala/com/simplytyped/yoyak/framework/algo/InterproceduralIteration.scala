...
import com.simplytyped.yoyak.framework.domain.mem.{
  MemDomLike,
  Resolvable,
  Localizable
}
...
trait InterproceduralIteration[A, D, M <: MemDomLike[A, D, M]]
    extends FlowSensitiveIteration[GaloisIdentity[M]]
    with CfgNavigator[GaloisIdentity[M]] {
  ...
  override def getNextBlocks(bb: BasicBlock): Seq[BasicBlock] = {
    ...
    methodSigOpt
      .flatMap { pgm.methods.get }
      .flatMap { _.cfg }
      .map { _.getNexts(bb).toList }
      .getOrElse(List.empty[BasicBlock])
  }
  override def memoryFetcher(
      map: MapDom[BasicBlock, GaloisIdentity[M]],
      b: BasicBlock
  ): Seq[M] = {
  ...
  override def work(
      ...
      input: M,
      block: BasicBlock
  ): (MapDom[BasicBlock, GaloisIdentity[M]], M) = {
  ...
  def mapArguments(invokeStmt: Invoke, input: M): M = {
    ...
    val newInput = argsWithIndex.foldLeft(input) { case (m, (v, i)) =>
      m.update(Param(i) -> v)
    }
    newInput
  }
  ...
  def invoking(
    ...
    targetMethods.foldLeft(map) { case (m, targetMethod) =>
      ...
      for (
        ...
        assert(
          nextBlocks.size == 1,
          "invoke block can only have one next block"
        )
        val nextBlock = nextBlocks.head
        val goEdge    = BasicEdge(block, targetEntry, InterEdge(None))
        val returnEdge =
          BasicEdge(targetExit, nextBlock, InterEdge(invokeStmt.ret))
        interproceduralEdges =
          interproceduralEdges.addEdge(goEdge).addEdge(returnEdge)
  ...
  def returning(
    ...
    exitNode.foldLeft(map) { case (m, exitNode) =>
