...
case class MapDom[D1, D2 <: AbsDomLike[D2]](t: Map[D1, D2])
  ...
  def <<=(other: MapDom[D1, D2]): Option[Boolean] = {
    if (t.size > other.t.size) {
      if (
        other.t.foldLeft(true) { case (b, (k, v)) =>
          t.get(k).flatMap { v <<= _ }.getOrElse(false) && b
        }
      ) Some(false)
      else None
    } else if (t.size < other.t.size) {
      if (
        t.foldLeft(true) { case (b, (k, v)) =>
          other.t.get(k).flatMap { v <<= _ }.getOrElse(false) && b
        }
      ) Some(true)
      else None
    } else {
      val (lt, rt) = t.foldLeft(true, true) { case ((l, r), (k, v)) =>
        val (l$, r$) = other.t
          .get(k)
          .map { x =>
            ((v <<= x).getOrElse(false), (x <<= v).getOrElse(false))
          }
  ...
  def ++(other: MapDom[D1, D2]) =
    t.foldLeft(other) { (m, pair) =>
      m.weakUpdate(pair._1, pair._2)
    }
}
