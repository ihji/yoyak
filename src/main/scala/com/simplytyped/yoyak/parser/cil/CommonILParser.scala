...
class CommonILParser extends JavaTokenParsers {
  def clazz: Parser[Clazz] =
    positioned(
      identifier ~ "{" ~ rep(method) ~ "}" ^^ { case name ~ _ ~ list ~ _ =>
        Clazz(name, list)
      }
    )
  def method: Parser[Method] =
    positioned(
      identifier ~ "(" ~ opt(identifierWithType) ~ rep(
        "," ~> identifierWithType
      ) ~ ")" ~ block ^^ { case name ~ _ ~ firstParam ~ params ~ _ ~ body =>
        Method(name, firstParam.toList ++ params, body)
      }
    )
  ...
  def ifstmt: Parser[If] =
    positioned(
      "if" ~ "(" ~ value ~ ")" ~ block ~ "else" ~ block ^^ {
        case _ ~ _ ~ v ~ _ ~ b1 ~ _ ~ b2 => If(v, b1, b2)
      }
    )
  def whilestmt: Parser[While] =
    positioned(
      "while" ~ "(" ~ value ~ ")" ~ block ^^ { case _ ~ _ ~ v ~ _ ~ b =>
        While(v, b)
      }
    )
  def assignstmt: Parser[Assign] =
    positioned(
      identifier ~ "=" ~ value ~ ";" ^^ { case lv ~ _ ~ rv ~ _ =>
        Assign(lv, rv)
      }
    )
  def invokestmt: Parser[Invoke] =
    positioned(
      opt(identifier <~ "=") ~ identifier ~ "(" ~ opt(value) ~ rep(
        "," ~> value
      ) ~ ")" ~ ";" ^^ { case ret ~ callee ~ _ ~ firstOpt ~ list ~ _ ~ _ =>
        Invoke(ret, callee, firstOpt.toList ++ list)
      }
    )
  ...
  def value: Parser[Value] =
    positioned(
      cinteger ~ othervalue ^^ { case lv ~ oprvOpt =>
        if (oprvOpt.nonEmpty) BinExp(lv, oprvOpt.get._1, oprvOpt.get._2)
        else lv
      } |
        cstring ~ othervalue ^^ { case lv ~ oprvOpt =>
          if (oprvOpt.nonEmpty) BinExp(lv, oprvOpt.get._1, oprvOpt.get._2)
          else lv
        } |
        identifier ~ othervalue ^^ { case lv ~ oprvOpt =>
          if (oprvOpt.nonEmpty) BinExp(lv, oprvOpt.get._1, oprvOpt.get._2)
          else lv
        }
    )
...
object CommonILParser {
  ...
  case class Method(name: Ident, params: List[(Ident, Type)], body: Block)
      extends Positional
  ...
  case class Invoke(ret: Option[Ident], callee: Ident, args: List[Value])
      extends CILStmt
