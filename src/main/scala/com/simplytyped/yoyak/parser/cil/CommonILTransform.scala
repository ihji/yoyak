...
class CommonILTransform {
  ...
  def transformExp(
      exp: CIL.BinExp
  ): (Value.BinExp, List[Statement.CoreStmt]) = {
    val (lv, lvStmt) = transformValue(exp.lv)
    val (rv, rvStmt) = transformValue(exp.rv)
    val op           = transformOp(exp.op)
    op match {
      case o: Value.BinOp.CompOp =>
        (Value.CompBinExp(lv, o, rv), lvStmt ++ rvStmt)
      case o: Value.BinOp.CondOp =>
        (Value.CondBinExp(lv, o, rv), lvStmt ++ rvStmt)
    }
  }
  ...
  def transformStmt(
      methodSigMap: Map[String, MethodSig]
  )(stmt: CIL.CILStmt): List[Statement.CoreStmt] = {
    stmt match {
      case CIL.If(cond, thenBlock, elseBlock) =>
        val (condLoc, condStmt) = transformValue(cond)
        val condExp =
          Value.CondBinExp(condLoc, Value.BinOp.!=, Value.BooleanConstant(true))
        ...
        val ifstmt =
          Statement.If(condExp, new Statement.Target().setStmt(targetNop))
        val gotostmt = Statement.Goto(new Statement.Target().setStmt(lastNop))
        condStmt ++ List(ifstmt) ++ thenB ++ List(
          gotostmt,
          targetNop
        ) ++ elseB ++ List(lastNop)
      case CIL.While(cond, loop) =>
        ...
        val ifstmt =
          Statement.If(condExp, new Statement.Target().setStmt(lastNop))
        val goback = Statement.Goto(new Statement.Target().setStmt(firstNop))
        List(firstNop) ++ condStmt ++ List(ifstmt) ++ loopB ++ List(
          goback,
          lastNop
        )
      ...
      case CIL.Invoke(ret, callee, args) =>
        val retLocalOpt =
          ret.map { transformValue }.map { _._1.asInstanceOf[Value.Local] }
        val targetSig = methodSigMap(callee.id)
        val (argV, argStmt) =
          args.foldLeft(List.empty[Value.Loc], List.empty[Statement.CoreStmt]) {
            case ((vl, sl), a) =>
      ...
      case CIL.Return(v) =>
        ...
        if (retLocStmt.nonEmpty)
          retLocStmt.get._2 ++ List(Statement.Return(Some(retLocStmt.get._1)))
        else List(Statement.Return(None))

    }
  }
  def transformMethod(clsName: ClassName, methodSigMap: Map[String, MethodSig])(
      mtd: CIL.Method
  ): (MethodSig, Method) = {
    val methodName = mtd.name.id
    val (paramAssigns, methodParamTypes) = mtd.params.zipWithIndex
      .foldLeft(List.empty[Statement.CoreStmt], List.empty[Type.ValueType]) {
        case ((al, tl), ((i, t), idx)) =>
          (
            al :+ Statement.Assign(Value.Local(i.id), Value.Param(idx)),
            tl :+ transformType(t)
          )
      }
  ...
  def transform(cls: CIL.Clazz): Clazz = {
    ...
    val methods = cls.methods.map {
      transformMethod(className, methodSigMap)
    }.toMap
