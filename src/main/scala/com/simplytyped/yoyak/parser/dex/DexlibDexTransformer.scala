...
class DexlibDexTransformer {
  def typeTransform(ty: String): Type.ValueType = {
    ty match {
      ...
      case _ if ty.startsWith("L") && ty.endsWith(";") =>
        Type.RefType(
          ClassName(ty.substring(1, ty.length - 1).replace("/", "."))
        )
    }
  }
  def instructionTransform(
      instrIdx: (Instruction, Int)
  )(implicit context: Context): Stmt = {
    ...
    instr.getOpcode match {
      ...
      case Opcode.MOVE | Opcode.MOVE_WIDE | Opcode.MOVE_OBJECT =>
        ...
        Assign(dest, src)
      case Opcode.MOVE_FROM16 | Opcode.MOVE_WIDE_FROM16 |
      ...
      case Opcode.MOVE_16 | Opcode.MOVE_WIDE_16 | Opcode.MOVE_OBJECT_16 =>
        ...
        val src  = getRegVar(move.getRegisterB)
        Assign(dest, src)
      case Opcode.MOVE_RESULT | Opcode.MOVE_RESULT_WIDE |
      ...
      case Opcode.CONST_STRING =>
        ...
        val dest =
          getRegVar(const.getRegisterA).setType(Type.CommonTypes.String)
        val constVal =
          const.getReference.asInstanceOf[StringReference].getString
      ...
      case Opcode.CONST_CLASS =>
        val const = instr.asInstanceOf[DexBackedInstruction21c]
        val dest  = getRegVar(const.getRegisterA)
        val ty = typeTransform(
          const.getReference.asInstanceOf[TypeReference].getType
        ).asInstanceOf[Type.ReferenceType]
      ...
      case Opcode.CHECK_CAST =>
        ...
        val ty = typeTransform(
          check.getReference.asInstanceOf[TypeReference].getType
        )
      ...
      case Opcode.INSTANCE_OF =>
        ...
        val ty = typeTransform(
          instanceOf.getReference.asInstanceOf[TypeReference].getType
        ).asInstanceOf[Type.ReferenceType]
      ...
      case Opcode.NEW_INSTANCE =>
        ...
        val ty = typeTransform(
          newInstance.getReference.asInstanceOf[TypeReference].getType
        ).asInstanceOf[Type.RefType]
      ...
      case Opcode.NEW_ARRAY =>
        ...
        val ty = typeTransform(
          newArray.getReference.asInstanceOf[TypeReference].getType
        )
      ...
      case Opcode.FILLED_NEW_ARRAY =>
        ...
        val args = List(
          getRegVar(newArray.getRegisterC),
          getRegVar(newArray.getRegisterD),
          getRegVar(newArray.getRegisterE),
          getRegVar(newArray.getRegisterF),
          getRegVar(newArray.getRegisterG)
        ).take(size)
        val stmts = Assign(
          methodReturnVar,
          NewArrayExp(ty, IntegerConstant(size))
        ) :: args.zipWithIndex.map { case (v, idx) =>
          Assign(ArrayRef(methodReturnVar, IntegerConstant(idx)), v)
        }
      ...
      case Opcode.FILLED_NEW_ARRAY_RANGE =>
        ...
        val args = (startRegister until startRegister + registerCount).map {
          getRegVar
        }.toList
        val stmts = Assign(
          methodReturnVar,
          NewArrayExp(ty, IntegerConstant(registerCount))
        ) :: args.zipWithIndex.map { case (v, idx) =>
          Assign(ArrayRef(methodReturnVar, IntegerConstant(idx)), v)
        }
        Block(new StatementContainer().setStmts(stmts))
      case Opcode.FILL_ARRAY_DATA =>
        ...
        val arrayDataList = context
          .instrs(targetIndex)
          .asInstanceOf[DexBackedArrayPayload]
          .getArrayElements
          .asScala
          .toList
        val stmts = arrayDataList.zipWithIndex.map { case (d, idx) =>
          Assign(
            ArrayRef(array, IntegerConstant(idx)),
            IntegerConstant(d.intValue)
          )
        }
        Block(new StatementContainer().setStmts(stmts))
      case Opcode.THROW =>
      ...
      case Opcode.GOTO =>
        ...
        val target = new Target().setStmt(
          Placeholder(context.instrs(offsetToIndex(idx, goto.getCodeOffset)))
        )
        Goto(target)
      case Opcode.GOTO_16 =>
      ...
      case Opcode.PACKED_SWITCH =>
        ...
        val payload =
          context.instrs(targetIndex).asInstanceOf[DexBackedPackedSwitchPayload]
      ...
      case Opcode.SPARSE_SWITCH =>
        ...
        val targetIndex = offsetToIndex(idx, switch.getCodeOffset)
        val payload =
          context.instrs(targetIndex).asInstanceOf[DexBackedSparseSwitchPayload]
        val table = payload.getSwitchElements.asScala.toList
        val revLists = table.foldLeft(List.empty[Int], List.empty[Int]) {
          case ((ks, vs), x) => (x.getKey :: ks, x.getOffset :: vs)
        }
      ...
      case Opcode.CMPL_FLOAT =>
        ...
        Assign(
          dest,
          CompBinExp(first, BinOp.cmpl, second).setType(Type.IntegerType)
        )
      case Opcode.CMPG_FLOAT =>
        ...
        Assign(
          dest,
          CompBinExp(first, BinOp.cmpg, second).setType(Type.IntegerType)
        )
      ...
      case Opcode.CMP_LONG =>
        ...
        Assign(
          dest,
          CompBinExp(first, BinOp.cmp, second).setType(Type.IntegerType)
        )
      case Opcode.IF_EQ =>
        ...
        val target = new Target().setStmt(
          Placeholder(context.instrs(offsetToIndex(idx, ifcond.getCodeOffset)))
        )
      ...
      case Opcode.AGET_BOOLEAN =>
        ...
        val base = getRegVar(get.getRegisterB).setType(
          Type.ArrayType(Type.BooleanType, 1)
        )
        val index = getRegVar(get.getRegisterC).setType(Type.IntegerType)
      ...
      case Opcode.AGET_BYTE =>
        ...
        val base =
          getRegVar(get.getRegisterB).setType(Type.ArrayType(Type.ByteType, 1))
        val index = getRegVar(get.getRegisterC).setType(Type.IntegerType)
      ...
      case Opcode.AGET_CHAR =>
        ...
        val base =
          getRegVar(get.getRegisterB).setType(Type.ArrayType(Type.CharType, 1))
        val index = getRegVar(get.getRegisterC).setType(Type.IntegerType)
      ...
      case Opcode.AGET_SHORT =>
        ...
        val base =
          getRegVar(get.getRegisterB).setType(Type.ArrayType(Type.ShortType, 1))
        val index = getRegVar(get.getRegisterC).setType(Type.IntegerType)
      ...
      case Opcode.APUT_BOOLEAN =>
        ...
        val base = getRegVar(put.getRegisterB).setType(
          Type.ArrayType(Type.BooleanType, 1)
        )
        val index = getRegVar(put.getRegisterC).setType(Type.IntegerType)
      ...
      case Opcode.APUT_BYTE =>
        ...
        val base =
          getRegVar(put.getRegisterB).setType(Type.ArrayType(Type.ByteType, 1))
        val index = getRegVar(put.getRegisterC).setType(Type.IntegerType)
      ...
      case Opcode.APUT_CHAR =>
        ...
        val base =
          getRegVar(put.getRegisterB).setType(Type.ArrayType(Type.CharType, 1))
        val index = getRegVar(put.getRegisterC).setType(Type.IntegerType)
      ...
      case Opcode.APUT_SHORT =>
        ...
        val base =
          getRegVar(put.getRegisterB).setType(Type.ArrayType(Type.ShortType, 1))
        val index = getRegVar(put.getRegisterC).setType(Type.IntegerType)
      ...
      case Opcode.IGET_BOOLEAN =>
        ...
        Assign(
          dest,
          InstanceFieldRef(base, fieldName).setType(Type.BooleanType)
        )
      ...
      case Opcode.SGET_BOOLEAN =>
        ...
        Assign(
          dest,
          StaticFieldRef(className, fieldName).setType(Type.BooleanType)
        )
      case Opcode.SGET_BYTE =>
        ...
        Assign(
          dest,
          StaticFieldRef(className, fieldName).setType(Type.ByteType)
        )
      case Opcode.SGET_CHAR =>
        ...
        Assign(
          dest,
          StaticFieldRef(className, fieldName).setType(Type.CharType)
        )
      case Opcode.SGET_SHORT =>
        ...
        Assign(
          dest,
          StaticFieldRef(className, fieldName).setType(Type.ShortType)
        )
      case Opcode.SPUT =>
      ...
      case Opcode.INVOKE_VIRTUAL | Opcode.INVOKE_SUPER | Opcode.INVOKE_DIRECT |
        ...
        val argTypes = method.getParameterTypes.asScala.map { x =>
          typeTransform(x.toString)
        }.toList
        val methodSig = MethodSig(className, methodName, argTypes)

        val argCount = invoke.getRegisterCount
        val args = List(
          getRegVar(invoke.getRegisterC),
          getRegVar(invoke.getRegisterD),
          getRegVar(invoke.getRegisterE),
          getRegVar(invoke.getRegisterF),
          getRegVar(invoke.getRegisterG)
        ).take(argCount)
        val packedArgs =
          removeSecondOfWideRegisters(argTypes, args.tail).zip(argTypes).map {
            case (r, ty) => r.setType(ty)
          }

        val retType = typeTransform(method.getReturnType)
        val retVar =
          if (retType == Type.VoidType) None
          else Some(methodReturnVar.setType(retType))
        ...
        Invoke(
          ...
          DynamicInvoke(
            methodSig,
            packedArgs,
            args.head.setType(Type.RefType(className))
          )
        )
      case Opcode.INVOKE_STATIC =>
        ...
        val packedArgs =
          removeSecondOfWideRegisters(argTypes, args).zip(argTypes).map {
            case (r, ty) => r.setType(ty)
          }
      ...
      case Opcode.INVOKE_VIRTUAL_RANGE | Opcode.INVOKE_SUPER_RANGE |
        ...
        val args = (startRegister until startRegister + regCount).map {
          getRegVar
        }.toList
        val packedArgs =
          removeSecondOfWideRegisters(argTypes, args.tail).zip(argTypes).map {
      ...
      case Opcode.NEG_INT =>
        ...
        Assign(
          dest,
          CompBinExp(IntegerConstant(0), BinOp.-, src).setType(Type.IntegerType)
        )
      case Opcode.NOT_INT =>
        val unary = instr.asInstanceOf[DexBackedInstruction12x]
        val dest  = getRegVar(unary.getRegisterA).setType(Type.IntegerType)
        ...
        Assign(
          dest,
          CompBinExp(src, BinOp.^, IntegerConstant(-1)).setType(
            Type.IntegerType
          )
        )
      case Opcode.NEG_LONG =>
        ...
        Assign(
          dest,
          CompBinExp(LongConstant(0), BinOp.-, src).setType(Type.LongType)
        )
      case Opcode.NOT_LONG =>
        ...
        Assign(
          dest,
          CompBinExp(src, BinOp.^, LongConstant(-1)).setType(Type.LongType)
        )
      case Opcode.NEG_FLOAT =>
        ...
        Assign(
          dest,
          CompBinExp(FloatConstant(0), BinOp.-, src).setType(Type.FloatType)
        )
      case Opcode.NEG_DOUBLE =>
        ...
        Assign(
          dest,
          CompBinExp(DoubleConstant(0), BinOp.-, src).setType(Type.DoubleType)
        )
      ...
      case Opcode.ADD_INT =>
        ...
        Assign(
          dest,
          CompBinExp(first, BinOp.+, second).setType(Type.IntegerType)
        )
      case Opcode.SUB_INT =>
        val binary = instr.asInstanceOf[DexBackedInstruction23x]
        val dest   = getRegVar(binary.getRegisterA).setType(Type.IntegerType)
      ...
      case Opcode.USHR_LONG =>
        ...
        Assign(
          dest,
          CompBinExp(first, BinOp.>>>, second).setType(Type.LongType)
        )
      ...
      case Opcode.ADD_DOUBLE =>
        ...
        Assign(
          dest,
          CompBinExp(first, BinOp.+, second).setType(Type.DoubleType)
        )
      ...
      case Opcode.ADD_FLOAT_2ADDR =>
        ...
        Assign(
          first,
          CompBinExp(first, BinOp.+, second).setType(Type.FloatType)
        )
      ...
      case Opcode.RSUB_INT =>
        ...
        Assign(
          dest,
          CompBinExp(second, BinOp.-, first).setType(Type.IntegerType)
        )
      ...
      case Opcode.IGET_VOLATILE | Opcode.IPUT_VOLATILE | Opcode.SGET_VOLATILE |
          Opcode.SPUT_VOLATILE | Opcode.IGET_OBJECT_VOLATILE |
          Opcode.IGET_WIDE_VOLATILE | Opcode.IPUT_WIDE_VOLATILE |
          Opcode.SGET_WIDE_VOLATILE | Opcode.SPUT_WIDE_VOLATILE |
          Opcode.THROW_VERIFICATION_ERROR | Opcode.EXECUTE_INLINE |
          Opcode.EXECUTE_INLINE_RANGE | Opcode.INVOKE_DIRECT_EMPTY |
          Opcode.INVOKE_OBJECT_INIT_RANGE | Opcode.RETURN_VOID_BARRIER |
          Opcode.IGET_QUICK | Opcode.IGET_WIDE_QUICK |
          Opcode.IGET_OBJECT_QUICK | Opcode.IPUT_QUICK |
          Opcode.IPUT_WIDE_QUICK | Opcode.IPUT_OBJECT_QUICK |
          Opcode.INVOKE_VIRTUAL_QUICK | Opcode.INVOKE_VIRTUAL_QUICK_RANGE |
          Opcode.INVOKE_SUPER_QUICK | Opcode.INVOKE_SUPER_QUICK_RANGE |
          Opcode.IPUT_OBJECT_VOLATILE | Opcode.SGET_OBJECT_VOLATILE |
          Opcode.SPUT_OBJECT_VOLATILE =>
        throw new NotSupportedException("odex not supported")

      case Opcode.PACKED_SWITCH_PAYLOAD | Opcode.SPARSE_SWITCH_PAYLOAD |
  ...
  def methodTransform(method: DexBackedMethod): Method = {
    ...
    Option(method.getImplementation)
      .map { impl =>
        val debugInfos =
          impl.getDebugItems.asScala.toList // TODO : annotate opcodes by making state machine

        val instrs    = impl.getInstructions.asScala.toList
        val offsetMap = buildOffsetMap(instrs)
        implicit val context =
          Context(offsetMap, offsetMap.map { _.swap }, instrs)
        ...
        def postProcess(stmts: List[Stmt]) = {
          ...
          val secondPass =
            CommonILHelper.stmtSubstitute(secondPassMap)(firstPass)
        ...
        val startingRegister =
          regCount - paramSizeList.reduceOption { _ + _ }.getOrElse(0)
        val thisStmts =
          if (!isStatic)
            List(
              Assign(
                getRegVar(startingRegister - 1)
                  .setType(Type.RefType(className)),
                This.setType(Type.RefType(className))
              )
            )
          else List.empty[CoreStmt]
        val paramStmts = paramSizeList
          .zip(params)
          .foldLeft(thisStmts, startingRegister, 0) {
            case ((stmt, regN, count), (size, paramTy)) =>
              val regVar = getRegVar(regN).setType(paramTy)
              (
                Assign(regVar, Param(count).setType(paramTy)) :: stmt,
                regN + size,
                count + 1
              )
          }
  ...
  def classTransform(clazz: DexBackedClassDef): Clazz = {
    ...
    val methodMap =
      methods.map { methodTransform }.map { x => (x.name, x) }.toMap
  ...
  def translate(dexFile: DexBackedDexFile): Program = {
    ...
    val classMap =
      classSet.map { classTransform }.map { x => (x.name, x) }.toMap
    Program(classMap)
  }
  private def removeSecondOfWideRegisters(
      paramTypes: List[Type.ValueType],
      args: List[Value.Loc]
  ): List[Value.Loc] = {
    paramTypes
      .map { CommonILHelper.getUnitSizeOf }
      .flatMap { case 1 => List(1); case 2 => List(1, 0) }
      .zip(args)
      .flatMap { case (mark, v) => if (mark == 1) Some(v) else None }
  }
  private def offsetToIndex(currentIdx: Int, offset: Int)(implicit
      context: Context
  ): Int = {
  ...
  private def buildOffsetMap(instrs: List[Instruction]): Map[Int, Int] = {
    instrs
      .foldLeft(Map.empty[Int, Int], 0, 0) { case ((m, idx, offset), instr) =>
...
object DexlibDexTransformer {
  ...
  case class Context(
      offsetToIndexMap: Map[Int, Int],
      /* map from codeOffset to instruction list index */
