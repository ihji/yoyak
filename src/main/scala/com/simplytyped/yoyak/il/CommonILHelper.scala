...
object CommonILHelper {
  ...
  def stmtSubstituteCore(
      map: Map[Stmt, CoreStmt]
  )(stmts: List[CoreStmt]): List[CoreStmt] = {
  ...
  private def substituteI(
      f: Value.Local => Value.Local
  )(v: Value.Instant): Value.Instant = {
    v match {
      case v: Loc => substituteL(f)(v)
      case _      => v
    }
  }
  private def substituteV(
      f: Value.Local => Value.Local
  )(v: Value.t): Value.t = {
    v match {
      ...
      case v: CondBinExp =>
        v.copy(lv = substituteV(f)(v.lv), rv = substituteV(f)(v.rv))
      case v: CompBinExp =>
        v.copy(lv = substituteV(f)(v.lv), rv = substituteV(f)(v.rv))
      case _ => v
    }
  }
  private def substituteL(
      f: Value.Local => Value.Local
  )(v: Value.Loc): Value.Loc = {
    v match {
      case v: Local => f(v)
      case v: ArrayRef =>
        v.copy(base = substituteL(f)(v.base), index = substituteI(f)(v.index))
      case v: InstanceFieldRef => v.copy(base = substituteL(f)(v.base))
      case v: StaticFieldRef   => v
      case v: Param            => v
    }
  }
  def substituteLocalDef(
      f: Value.Local => Value.Local
  )(stmt: CoreStmt): CoreStmt = {
    stmt match {
      case stmt @ Identity(lv, rv) =>
        StmtCopier.Identity(stmt, f(lv), rv)
      case stmt @ Assign(lv, rv) =>
        val newLv =
          if (lv.isInstanceOf[Local]) f(lv.asInstanceOf[Local]) else lv
    ...
    stmt match {
      ...
      case stmt @ Invoke(ret, callee) =>
        val newCallee = callee match {
          case inv: DynamicInvoke =>
            inv.copy(
              args = inv.args.map { substituteL(f) },
              base = substituteL(f)(inv.base)
            )
          case inv: StaticInvoke =>
            inv.copy(args = inv.args.map { substituteL(f) })
        }
