...
import com.simplytyped.yoyak.il.hierarchy.ClassHierarchy.{Inheritance, TypeDef, HierarchyGraph}

class ClassHierarchy {
  private var graph = HierarchyGraph.empty

  def extendsFrom(childClass: ClassName, parentClass: ClassName) : ClassHierarchy = {
    graph = graph.addEdge(TypeDef(childClass),TypeDef(parentClass))
    this
  }

  def implementWith(childClass: ClassName, superInterface: ClassName) : ClassHierarchy = {
  ...
  def isImplementWith(childClass: ClassName, superInterface: ClassName) : Boolean = {
...
object ClassHierarchy {
  case class HierarchyGraph(nodes: Set[TypeDef], edges: Set[Inheritance], nexts: Map[TypeDef,Set[Inheritance]], prevs: Map[TypeDef,Set[Inheritance]]) extends ImmutableGraphLike[TypeDef,Inheritance,HierarchyGraph] {
    def newEdge(from: TypeDef, to: TypeDef): Inheritance = Inheritance(from,to)
    def builder(nodes: Set[TypeDef], edges: Set[Inheritance], nexts: Map[TypeDef, Set[Inheritance]], prevs: Map[TypeDef, Set[Inheritance]]): HierarchyGraph = {
