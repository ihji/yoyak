...
class VarSplitting {
    ...
    val defGroup = defMap
      .foldLeft(Map.empty[CoreStmt, MSet[CoreStmt]]) {
        case (defGroupMap, (_, m)) =>
          m.foldLeft(defGroupMap) { case (defGroupMap, (_, stmtSet)) =>
            val existingStmt = stmtSet.find { s =>
              defGroupMap.get(s).nonEmpty
            }
  ...
  private def findNewLocal(
      defMap: MapDom[Local, SetCoreStmt]
  )(old: Local): Local = {
    ...
    if (stmts.isEmpty) old
      ...
      if (newLocalOpt.nonEmpty)
        newLocalOpt.get.setType(Type.lub(old.ty, newLocalOpt.get.ty))
      else {
        assert(stmts.size == 1)
  ...
  private def renameLocal(
      defMap: MapDom[Local, SetCoreStmt]
  )(stmt: CoreStmt): (MapDom[Local, SetCoreStmt], CoreStmt) = {
    ...
    val newStmt2 =
      CommonILHelper.substituteLocalDef(findNewLocal(nextMap))(newStmt1)
    (nextMap, newStmt2)
  }
  def run(cfg: CFG): CFG = {
    ...
    cfg.nodes.foreach { bb =>
      ...
      val defMap = prevs.foldLeft(DefReachability.ops.bottom) { (x, y) =>
        DefReachability.ops.\/(x, defreach.get(y))
      }
      val stmts = bb.data.getStmts
      val newStmts = stmts
        .foldLeft(defMap, List.empty[CoreStmt]) { case ((map, list), s) =>
