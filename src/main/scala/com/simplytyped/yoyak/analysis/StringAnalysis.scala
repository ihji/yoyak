...
class StringAnalysis(cfg: CFG) {
  def run(): List[String] = {
    ...
    val analysis = new FlowSensitiveForwardAnalysis[
      GaloisIdentity[MemDom[SetInt, SetAbstraction[String]]]
    ](cfg)
  ...
  def findHost(
      stmt: Stmt,
      mem: MemDom[SetInt, SetAbstraction[String]]
  ): List[String] = {
    stmt match {
      case Invoke(_, callee) if AndroidAPIs.internet(callee.callee) =>
        callee.args.foldLeft(List.empty[String]) { case (list, v) =>
          v match {
            case l: Loc =>
              mem.get(l) match {
  ...
  def check(
      output: MapDom[BasicBlock, GaloisIdentity[
        MemDom[SetInt, SetAbstraction[String]]
      ]]
  ): List[String] = {
    cfg.nodes.foldLeft(List.empty[String]) { (list, node) =>
      val prevs = cfg.getPrevs(node)
      val input = prevs.foldLeft(StringAnalysis.memDomOps.bottom) { (m, p) =>
        StringAnalysis.memDomOps.\/(m, output.get(p))
      }
      val hosts = node.data.getStmts
        .foldLeft(input, List.empty[String]) { case ((m, l), stmt) =>
...
object StringAnalysis {
  ...
  implicit val absTransfer: StdSemantics[SetInt, SetAbstraction[String], MemDom[
    SetInt,
    SetAbstraction[String]
  ]] = new StdSemantics[
    SetInt,
    SetAbstraction[String],
    MemDom[SetInt, SetAbstraction[String]]
    ...
    override protected def evalConstant(
        v: Value.Constant,
        input: MemDom[SetInt, SetAbstraction[String]]
    )(implicit context: Context) = {
      v match {
        case StringConstant(s) =>
          (AbsBox[SetAbstraction[String]](Set(s)), input)
        case _ => (AbsTop, input)
      }
    }
  }
  ...
  implicit val memDomOps
      : LatticeOps[GaloisIdentity[MemDom[SetInt, SetAbstraction[String]]]] =
    MemDom.ops[SetInt, SetAbstraction[String]]
}
