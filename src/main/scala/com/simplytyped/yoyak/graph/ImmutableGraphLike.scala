...
/** trait for immutable graphs
  */
trait ImmutableGraphLike[Node <: NodeLike[Node], Edge <: EdgeLike[
  Node
], Graph <: GraphLike[Node, Edge, Graph]]
    extends GraphLike[Node, Edge, Graph] { self: Graph =>
  ...
  def removeEdge(e: Edge): Graph = {
    ...
    val newNexts =
      if (newNextSet.isEmpty) nexts - e.from else nexts + (e.from -> newNextSet)
    val newPrevSet = prevs(e.to) - e
    val newPrevs =
      if (newPrevSet.isEmpty) prevs - e.to else prevs + (e.to -> newPrevSet)
  ...
  def removeNode(n: Node): Graph = {
    ...
    (incomingEdges ++ outgoingEdges).foldLeft(
      builder(nodes - n, edges, nexts, prevs)
    ) { _.removeEdge(_) }
  }
  ...
  def getNexts(n: Node): Set[Node] = {
    nexts
      .get(n)
      .map { _.aggregate(Set.empty[Node])(_ + _.to, _ ++ _) }
      .getOrElse(Set.empty[Node])
  }
  def getPrevs(n: Node): Set[Node] = {
    prevs
      .get(n)
      .map { _.aggregate(Set.empty[Node])(_ + _.from, _ ++ _) }
      .getOrElse(Set.empty[Node])
  }
  ...
  def builder(
      nodes: Set[Node],
      edges: Set[Edge],
      nexts: Map[Node, Set[Edge]],
      prevs: Map[Node, Set[Edge]]
  ): Graph
}
