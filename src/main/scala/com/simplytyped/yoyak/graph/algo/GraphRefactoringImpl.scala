...
trait GraphRefactoringImpl[Node <: NodeLike[Node], Edge <: EdgeLike[
  Node
], Graph <: GraphLike[Node, Edge, Graph]]
    extends GraphRefactoring[Graph, Node] {
  def mergePairedNodes(merger: (Node, Node) => Node)(g: Graph): Graph = {
    ...
    val (_, finalG) = pairedEdges.foldLeft(Map.empty[Node, Node], g) {
      case ((remapper, g), e) =>
        ...
        val incomingNodesOfTwo =
          g.getPrevs(from).map { findMerged(newRemapper) }
        val vanillaG = g.removeNode(to).removeNode(from)
        val incomingAdded = incomingNodesOfTwo.foldLeft(vanillaG) {
          _.addEdge(_, newNode)
        }
        val newG = outgoingNodesOfTwo.foldLeft(incomingAdded) {
          _.addEdge(newNode, _)
        }
        (newRemapper, newG)
    }
