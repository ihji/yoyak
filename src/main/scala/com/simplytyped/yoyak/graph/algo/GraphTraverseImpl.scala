...
trait GraphTraverseImpl[Node <: NodeLike[Node], Edge <: EdgeLike[
  Node
], Graph <: GraphLike[Node, Edge, Graph]]
    extends GraphTraverse[Graph, Node] {
  def depthFirstTraverse(g: Graph): Stream[Node] = {
    def depthFirstTraverse_aux(
        visiting: Node,
        visited: Set[Node],
        notVisited: List[Node]
    ): Stream[Node] = {
      ...
      if (newNotVisited.isEmpty) visiting #:: Stream.empty[Node]
        ...
        visiting #:: depthFirstTraverse_aux(
          newNotVisited.head,
          visited + visiting,
          newNotVisited.tail
        )
  ...
  def findLoopheads(g: Graph): Vector[Node] = {
    def findLoopheads_aux(visiting: Node, visited: Set[Node]): Vector[Node] = {
      val nexts = g.getNexts(visiting).toList
      nexts.foldLeft(Vector.empty[Node]) { case (l, n) =>
        if (visited(n)) l :+ n
    ...
    if (roots.isEmpty) Vector.empty[Node]
      ...
      roots
        .foldLeft(Vector.empty[Node]) { case (l, n) =>
          l ++ findLoopheads_aux(n, Set())
        }
        .distinct
  }
}
