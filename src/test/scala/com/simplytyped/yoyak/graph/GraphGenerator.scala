...
object GraphGenerator {
  ...
  case class IntegerImmutableGraph(
      nodes: Set[IntNode],
      edges: Set[IntEdge],
      nexts: Map[IntNode, Set[IntEdge]],
      prevs: Map[IntNode, Set[IntEdge]]
  ) extends ImmutableGraphLike[IntNode, IntEdge, IntegerImmutableGraph] {
  ...
  object IntegerImmutableGraph {
    val empty =
      IntegerImmutableGraph(Set.empty, Set.empty, Map.empty, Map.empty)
  ...
  def joinGraphGen(count: Int): Gen[IntegerImmutableGraph] =
    for {
      _           <- arbitrary[Int]
      ...
      (first ++ second).addEdge(
        first.nodes.toList(firstIndex),
        second.nodes.toList(secondIndex)
      )
    }
