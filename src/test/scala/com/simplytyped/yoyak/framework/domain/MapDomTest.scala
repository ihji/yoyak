...
class MapDomTest extends FunSuite with Matchers {
  test("MapDom ordering (<=) test: unrelated") {
    ...
    val map1 = MapDom.empty[Int,Set[Int]]
    map1.update(1->Set(1,2,3))
    val map2 = MapDom.empty[Int,Set[Int]]
    map2.update(2->Set(1,2,3))
    val ordering = MapDom.ops[Int,Set[Int]]

    ordering.<=(map1,map2) should be (false)
  }
  test("MapDom ordering (<=) test: lhs is bigger") {
    import MapDomTest.ops
    val map1 = MapDom.empty[Int,Set[Int]]
    map1.update(1->Set(1,2,3,4))
    val map2 = MapDom.empty[Int,Set[Int]]
    map2.update(1->Set(1,2,3))
    val ordering = MapDom.ops[Int,Set[Int]]
  ...
  test("MapDom ordering (<=) test: rhs is bigger") {
    ...
    map1.update(1->Set(1,2,3))
    val map2 = MapDom.empty[Int,Set[Int]]
    map2.update(1->Set(1,2,3,4))
    val ordering = MapDom.ops[Int,Set[Int]]

    ordering.<=(map1,map2) should be (true)
  }
  test("MapDom ordering (<=) test: lhs has more elements") {
    import MapDomTest.ops
    val map1 = MapDom.empty[Int,Set[Int]]
    map1.update(1->Set(1,2)).update(2->Set(2,3,4))
    val map2 = MapDom.empty[Int,Set[Int]]
    map2.update(1->Set(1,2,3))
    val ordering = MapDom.ops[Int,Set[Int]]
  ...
  test("MapDom ordering (<=) test: rhs has more elements") {
    ...
    map1.update(1->Set(1,2,3))
    val map2 = MapDom.empty[Int,Set[Int]]
    map2.update(1->Set(1,2,3)).update(2->Set(2,3,4))
    val ordering = MapDom.ops[Int,Set[Int]]
...
object MapDomTest {
  implicit val ops : LatticeOps[Set[Int]] = new LatticeOps[Set[Int]] {
    override def <=(lhs: Set[Int], rhs: Set[Int]): Boolean =  lhs.subsetOf(rhs)
